<div class="row mt-3  mx-auto ">
    <div class="col-md-6   ">
        <a href="{{url('panel/coach/create')}}" class="my-1  d-block ">
    ...
    <search-coaches
            price="{{\App\Models\Setting::firstOrNew(['key'=>'coach_price'])->value?:0}}"
            admin="{{$admin}}"
            panel="{{true}}"
            type="{{Helper::$typesMap['coaches']}}"
            img-link="{{asset("storage/")}}"
            asset-link="{{asset("img/")}}"
            edit-link="{{route('coach.edit')}}"
            remove-link="{{route('coach.remove')}}"
