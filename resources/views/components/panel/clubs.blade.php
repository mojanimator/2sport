<div class="row mt-3  mx-auto ">
    <div class="col-md-6   ">
        <a href="{{url('panel/club/create')}}" class="my-1  d-block ">
    ...
    <search-clubs
            price="{{\App\Models\Setting::firstOrNew(['key'=>'club_price'])->value?:0}}"
            admin="{{$admin}}"
            panel="{{true}}"
            type="{{Helper::$typesMap['clubs']}}"
            img-link="{{asset("storage/")}}"
            asset-link="{{asset("img/")}}"
            edit-link="{{route('club.edit')}}"
            remove-link="{{route('club.remove')}}"
