@php
    $user=auth()->user();
      if($user->role=='ad' || $user->role=='go')
        ...
        $product = \App\Models\Product::whereIn('shop_id', \App\Models\Shop::where('user_id',$user->id)->pluck('id'))->where('id', $param)->with('docs:id,type_id,docable_id')->first();
...
@endphp
{{--if (Illuminate\Support\Facades\Gate::allows('ownItem',  [\App\Models\User::class, $shop, false]  )){--}}
